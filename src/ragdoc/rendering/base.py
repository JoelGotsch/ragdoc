...
if TYPE_CHECKING:
    from ragdoc.document import BaseElement, Document, ElementType
...
class ExternalRefProvider(Protocol):
    ...
    def get(
        self, element_id: str, default: Any = None
    ) -> "Document | BaseElement | None":
        ...
        if header:
            ...
            if dl:
                for dt, dd in zip(dl.find_all("dt"), dl.find_all("dd")):
                    head_parts.append(
                        f'<meta name="{dt.get_text()}" content="{dd.get_text()}"/>'
                    )
            header.extract()
            full_html = (
                f"<html><head>{''.join(head_parts)}</head>"
                f"<body>{soup}</body></html>"
            )
            result = convert_text(
                full_html, to=self.format.value, format="html", extra_args=["--standalone"]
            )
        else:
