...
def compute_size_to_level_mapping(
    ...
    unique_sizes = sorted(
        set(s for s in effective_sizes if s > 0),
        reverse=True
    )
...
class HeadingVisualInfo(BaseModel):
    ...
    index: int = Field(
        ...,
        description="Index of the heading in the document's elements list"
    )
    font_size: float | None = Field(
        default=None,
        description="Font size in points, extracted from CSS. None if not available."
    )
    is_all_caps: bool = Field(
        default=False,
        description="Whether the heading text is all uppercase"
    )
    is_centered: bool = Field(
        default=False,
        description="Whether the heading is centered (from CSS text-align)"
    )
    is_bold: bool = Field(
        default=False,
        description="Whether the heading is bold (from CSS or <b>/<strong> tags)"
    )
...
class HeadingLevelProcessor(DocumentProcessor):
    ...
    async def process(self, document: "Document") -> "Document":
        ...
        if self.trust_parser_levels and document.parser in ("html", "pandoc"):
            logger.debug(
                f"HeadingLevelProcessor: skipping (parser={document.parser!r}, trust_parser_levels=True)"
            )
            return document
        ...
        logger.debug(
            f"HeadingLevelProcessor: {len(heading_infos)} headings, {len(size_to_level)} distinct sizes"
        )
    ...
    def _collect_heading_infos(
        ...
        for idx, element in enumerate(document.elements):
            ...
            infos.append(HeadingVisualInfo(
                index=idx,
...
class TitleDetectionProcessor(DocumentProcessor):
    ...
    def _recalculate_heading_levels(
        self, document: "Document", removed_title_level: int
    ) -> None:
...
def normalize_heading_levels(document: "Document") -> "Document":
    ...
    levels_present = sorted(
        {el.level for el in document.elements if isinstance(el, Heading) and el.level > 0}
    )
